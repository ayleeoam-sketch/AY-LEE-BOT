...
export const PROVIDERS = {
  ...
  openrouter: {
    ...
    headers: { 'HTTP-Referer': 'https://github.com', 'X-Title': 'VENOM MD BOT' }
  },
  mistral: {
    label: 'Mistral',
    env: 'MISTRAL_API_KEY',
...
export async function loadKeys() {
  try {
    ...
    for (const row of rows) {
      if (row.key?.endsWith('_API_KEY') && row.value) runtimeKeys.set(row.key, row.value)
    }
  } catch {}
}
...
async function callGemini(p, key, messages, opts) {
  ...
  const body = { contents, generationConfig: { maxOutputTokens: opts.maxTokens || 1500, temperature: opts.temperature ?? 0.7 } }
  if (system) body.systemInstruction = { parts: [{ text: system }] }

  const { data } = await axios.post(
    `https://generativelanguage.googleapis.com/v1beta/models/${opts.model || p.model}:generateContent?key=${key}`,
    body,
    { headers: { 'Content-Type': 'application/json' }, timeout: opts.timeout || 90_000 }
  )
  const out = data?.candidates?.[0]?.content?.parts?.map((x) => x.text).join('')
  if (!out?.trim()) {
    const blocked = data?.promptFeedback?.blockReason
    throw new Error(blocked ? `blocked: ${blocked}` : 'empty response')
  }
  return out.trim()
}
...
async function callCohere(p, key, messages, opts) {
  const { data } = await axios.post(
    p.url,
    { model: opts.model || p.model, messages, max_tokens: opts.maxTokens || 1500 },
    { headers: { Authorization: `Bearer ${key}`, 'Content-Type': 'application/json' }, timeout: opts.timeout || 90_000 }
  )
  const out = data?.message?.content?.map((c) => c.text).join('')
  if (!out?.trim()) throw new Error('empty response')
  return out.trim()
}
...
async function callFree(messages, opts) {
  ...
  for (const attempt of attempts) {
    try {
      ...
      if (out?.trim() && !out.includes('Payment Required')) return out.trim()
    } catch {}
  }
  throw new Error('free endpoints are rate-limited or paywalled')
}

async function callProvider(name, messages, opts = {}) {
  const p = PROVIDERS[name]
  const key = getKey(name)
  if (!p || !key) throw new Error(`${name}: no key`)
  if (p.dialect === 'gemini') return callGemini(p, key, messages, opts)
  if (p.dialect === 'cohere') return callCohere(p, key, messages, opts)
...
/**
 * Ask the AI. Tries the preferred provider, then every other keyed provider
...
export async function chat(messages, opts = {}) {
  ...
  const chain = []
  if (opts.provider && getKey(opts.provider)) chain.push(opts.provider)
  for (const name of available()) if (!chain.includes(name)) chain.push(name)

  const errors = []
  for (const name of chain) {
    try {
      const text = await callProvider(name, messages, opts)
      return { text, provider: PROVIDERS[name].label, model: opts.model || PROVIDERS[name].model }
    } catch (e) {
      const status = e.response?.status
      const detail = e.response?.data?.error?.message || e.message
      errors.push(`${PROVIDERS[name].label}: ${status || ''} ${String(detail).slice(0, 60)}`)
    }
  }
  ...
  if (!opts.noFallback) {
    try {
      const text = await callFree(messages, opts)
      return { text, provider: 'Free (keyless)', model: 'pollinations' }
    } catch (e) {
      errors.push(`Free: ${e.message}`)
    }
  }
  ...
  const err = new Error(
    chain.length
      ...
      : 'No AI key is set, and the free keyless services are now paywalled or ' +
        ...
        '_Run *.aikeys* for all 8 providers._'
  )
...
export default { PROVIDERS, chat, ask, available, getKey, setKey, clearKey, loadKeys, history, remember, forget }
